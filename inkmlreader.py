...
class inkML(object):
    ...
    def expr_size_scaling2(self):
        ...
        for ctr,strk in enumerate(self._strokes):
            ...
            # xh,yh = (strk.coords[:,0].max() - strk.coords[:,0].min(),
            #           strk.coords[:,1].max() - strk.coords[:,1].min())
            new_coords = []
    ...
    def get_relation_candids(self):
        ...
        for i, grp in enumerate(self.stroke_groups):
            # TODO: ensure there's no diff b/n '-' and the larger '-' one
            if grp.target == '-' or grp.prediction == '-': 
                limit = max(0, i-1)
