...
class StrokeGroup(object):
    ...
    def bounding_box(self):
        ...
        for strk in self.strokes[1:]:
            try:
                ...
                print("!!! Warning in bounding box: " + str(e))
    ...
    def get_extra_features(self):
        ...
        if len(distances) == 0:
            distances = [-99]
        return [np.mean(angle_means),np.mean(angle_vars),np.sum(distances),
                np.mean(curv_means),np.mean(curv_vars)]
...
class Stroke(object):
    ...
    def _slope(self):
        slopes = []
        for i,row in enumerate(self.coords.T[2:]):
            slopes.append(curve(self.coords.T[i-2]+np.array([1,0]),
                self.coords.T[i-2],self.coords.T[i+2]))
